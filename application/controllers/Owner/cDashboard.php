...
class cDashboard extends CI_Controller
{
	...
	public function cetak_laporan_tahun()
	{
		require('asset/fpdf/fpdf.php');
		...
		$pdf->SetFont('Times', 'B', 14);
		$pdf->Cell(190, 10, 'APOTEK AGRA MEDIKA', 0, 1, 'C');
		$pdf->Cell(190, 10, 'LAPORAN TRANSAKSI TAHUN ' . $tahun, 0, 1, 'C');
		...
		$data = $this->db->query("SELECT * FROM `transaksi_obat` LEFT JOIN pelanggan ON transaksi_obat.id_pelanggan=pelanggan.id_pelanggan WHERE YEAR(tgl_transaksi)='" . $tahun . "' AND stat_transaksi='4'")->result();
		$total = 0;
		foreach ($data as $key => $value) {
			if ($value->point_transaksi == '0') {
				$pelanggan = 'Transaksi Langsung';
